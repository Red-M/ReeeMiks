...
func (d *Reeemiks) initializeTray(onDone func()) {
	logger := d.logger.Named("tray")

	onReady := func() {
		logger.Debug("Tray instance ready")

		systray.SetTemplateIcon(icon.ReeemiksLogo, icon.ReeemiksLogo)
		systray.SetTitle("reeemiks")
		systray.SetTooltip("reeemiks")
		...
		systray.AddSeparator()
		quit := systray.AddMenuItem("Quit", "Stop reeemiks and quit")
