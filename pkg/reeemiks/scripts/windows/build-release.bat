...
ECHO Building reeemiks (release)...
...
go build -o "%REEEMIKS_ROOT%\reeemiks-release.exe" -ldflags "-H=windowsgui -s -w -X main.gitCommit=%GIT_COMMIT% -X main.versionTag=%VERSION_TAG% -X main.buildType=%BUILD_TYPE%" "%REEEMIKS_ROOT%\pkg\reeemiks\cmd"
IF %ERRORLEVEL% NEQ 0 GOTO BUILDERROR
ECHO Done.
GOTO DONE

:BUILDERROR
ECHO Failed to build reeemiks in release mode! See above output for details.
EXIT /B 1
